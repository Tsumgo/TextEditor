...
//  ???????2019??2??28??
//            ????????????????????????
//            ??????????????????????demoGuiALL.c??drawButtons??????
//  ???????2019??2??26??
//            ?????????????????
//            ????????????
//            ??????textbox ?????????
//            ??????????
//            ?? uiGetInput ? uiGetMouse,uiGetKey,uiGetKeyboard
//  ???????2019??2??18??
//  ??????????2018??4???????<<??????????>>??????
//
//===========================================================================
...
#define KMOD_SHIFT 0x01 // shift = 1
#define KMOD_CTRL 0x02	// ctrl = 2
...
// 	int keyModifiers; //  key modifier (shift = 1, ctrl = 2, shift+ctrl = 3)
// } UIState;
...
int menuList(int id, double x, double y, double w, double wlist, double h, char *labels[], int n)
{
	...
	if (inBox(gs_UIState.mousex, gs_UIState.mousey, x, x + w, y, y + h)) // 上一个事件后，鼠标在这个box内
		gs_UIState.actingMenu = id;

	if (menuItem(id, x, y, w, h, labels[0])) // 鼠标点击了当前menu
...
// int TextBox(int id, double x, double y, double w, double h, char textbuf[], int fuflen)
